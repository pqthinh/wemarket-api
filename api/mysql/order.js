...
const Order = {
    ...
    BuyerCreateOrder: async (req, res, next) => {
        ...
        try {
            ...
            let title = `Người dùng ${user.username} đã yêu cầu mua sản phẩm ${product.code}`;
    ...
    SellerAcceptOrder: async (req, res, next) => {
        ...
        try {
            ...
            where id = ?`;
            result = await conn.query(sqlUpdateProduct, [newQuantity, newStatus, updatedAt, order.product_id]);
            await conn.commit();
            //create notify
            ...
            let title = `Người dùng ${order.sellerUsername} đã chấp nhận yêu cầu mua sản phẩm ${order.code} của bạn`;
    ...
    SellerUpdateOrder: async (req, res, next) => {
        ...
        try {
            ...
            let month = updatedAt.getMonth() + 1;
            let year = updatedAt.getFullYear();
            let title = `Người dùng ${order.sellerUsername} đã sửa yêu cầu mua sản phẩm ${order.code} của bạn`;
    ...
    SellerDeclineOrder: async (req, res, next) => {
        ...
        try {
            ...
            let month = updatedAt.getMonth() + 1;
            let year = updatedAt.getFullYear();
            let title = `Người dùng ${order.sellerUsername} đã từ chối yêu cầu mua sản phẩm ${order.code} của bạn`;
