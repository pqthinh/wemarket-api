...
const Product = {
  ...
  createProduct: async (req, res, next) => {
    ...
    try {
      ...
      //Get images
            imgs = await conn.query("select * from image where productId=?", [id,]);
    ...
    try {
      ...
      if(images.length > 10) {
        ...
        if(hasUser[0].length < 1) {
          ...
          //update image
          await conn.query('update image set deletedAt = ?, updatedAt = ? where productId = ?',[updatedAt, updatedAt, idProduct]);
          for(let img of images) {
            let imgQuery = await conn.query('Select * from image where url = ? AND productId = ?', [
              img,
              idProduct
            ]);
            await conn.commit();
  ...
  deleteProduct: async (req, res, next) => {
    ...
    try {
      ...
      //delete image
      await conn.query('update image set deletedAt = ?, updatedAt = ? where productId = ?',[deletedAt, deletedAt, idProduct]);
      await conn.commit();
  ...
  adminActivePost: async (req, res, next) => {
    ...
    try {
      ...
      //Active image
      await conn.query('update image set status = "active", updatedAt = ? where productId = ?',[updatedAt, idProduct]);
      await conn.commit();
